...
`Aggregation`: A class contains another class (a course contains students). Diamond is at the end with the parent class (course).
...
All subtypes should be substitutable for their base types
...
- Example: Selecting type of transport on google maps
...
<img src="./images/ob.png" width="700" height="auto">
...
abstract class Department {
    abstract Employee createEmployee(int id);
}
...
public class FactoryMethodExample {
    public static void main(String[] args) {
        ...
        Employee programmer = itDept.createEmployee(123);
        programmer.work();

        Department accountingDept = new AccountingDepartment();
        Employee accountant = accountingDept.createEmployee(456);
        accountant.work();
    }
}
```
...
assertEquals(bubbleTree.decorate(), 
...
- Limited logic around chaning strategies

2. State
- States
- Action -> cause transitions
- Logic around actions 

3. Composite 
- Leaf nodes 
- Tree hierachy
- Leaf and Composite nodes considered same 

4. Observer 
- Being observed (subject, observable, publisher)
- Doing observing (observer, subscriber, consumer, listener)
- Method of notification 
  - push, notification sent from subject (teacher sending marks)
  - pull, observer manually checking (student checking for marks, ticket checking bot)
- A change in state causes the updating/notification

5. Factory
- Create other objects - logic around creation
- Abstract create families of objects

6. Decorator
- Dynamically add functionality
- Layering/wrapping
- Decorate to interface, not implementation

7. Template
- Series of steps 
- Some steps abstractly defined, some concretely defined
- 'Hook' methods (bit of template you need to fill out)

8. Visitor 
- Seperate algorithm and implementation
- Add functionality to an exisiting system (legacy)
...
## Revision Exercises
1) A, Aggregation because tyres can exist by themselves
2) B, consumer does not return anything
3) C, Builder follow the stages step by step 
4) B, Open to extension, closed to modification
5) D
6) 
    ```java
    Object o;
    ...
    l1.add(c); // doesn't compile
    l2.add(c); // compiles
    l1.add(s); // doesn't compile
    l2.add(s); // compiles
    l1.add(o); // doesn't compile
    l2.add(o); // doesn't compile
    c = l1.get(0); // doesn't compile
    c = l2.get(0); // doesn't compile
    s = l1.get(0); // compiles
    s = l2.get(0); // doesn't compile
    o = l1.get(0); // compiles
    o = l2.get(0); // compiles
    ```
    Firstly, it is important to note that l1 can be a List that stores any type that extends Shape. It could be a List of Circles or a List of RedCircles, etc etc. Therefore, it is impossible to know at compile time whether you can add any type to l1 as it might not be a List that accepts that type. Therefore, all lines that try to add to l1 will produce a compile error (lines 1,3,5).

    Line 6 (l2.add(o)) will also produce a compile error. This is because we cannot be certain that adding an Object to l2 is valid. For instance, what if l2 was a List<Shape>, then adding an Object would be invalid. However adding either a Circle or a Shape is fine as you can be certain that whatever type the List is, it is a superclass of both Circle and Shape and so both of those objects can simply be upcasted when adding to the list.

    So, in the first half of the problem. This gives us 4 compile errors.

    The other half of the problem is largely the reverse with one small change. Firstly, it is impossible to get from l2 as the objects in l2 could be a parent class of whatever type you are trying to get. i.e. trying to get a Circle from l2 may be invalid as l2 may be a List<Object>. There is one exception to this. Since all objects must be at least type Object, the last line (o = l2.get(0)) is valid since we can be certain that it must be at least an Object type. This gives us compile errors on lines 8 and 10.

    Lastly, trying to get either an Object or Shape from l1 is fine because we can be certain that anything in that list must be a subtype of those classes. However, trying to get a Circle is not valid as the list may not only contain type Circle . This gives us a compile error on line 7.
7) D
8) B
9)  B
10) D
...
## Part B
### q1
1. Identify the logic error present (1 mark)
...
public boolean equals(Object obj) {
    ...
    // instanceof allows subclasses = superclasses
    // if (!(obj.getClass() == Article.class)) return false;
...
```java
 ...
 // This would return true, however it should return false
    assertFalse(article.equals(new News()))
```

```java
// runtime error, removing elements from a list that you are iterating over it
...
### q3
The design pattern that should be used to model the different Jira boards is the template pattern. Since all three boards have the same general workflow, this can be placed in the skeleton method of the superclass in which all three boareds can extends/implement. However there are minor differences in sprint times and these specific features can be delegated to methods within the subclass. The reason it is not strategy pattern is because once the board has been selected it cannot be changed, strategy allows users to dynamically change strategy or types of boards, where as template pattern has static methods which cannot be changed.

### q4
### a
Covariance refers to how an objects sub-classes has the ability to act and exist as the object itself. An example would be when you are returning an object of type shape in the superclass, you can return an object of type circle in the subclass.

Contravariance refers to broadening the scope of an object. Changing from a sub-class to a super-class. An example would be returning circle in the superclass, but returning shape in the subclass. Here this is invalid as it broadens the return type.

### b
contravariance

### c
The `bar` method in the class `Foo` post-condition guarantees that the return type must an `ArrayList`. However in `Foo2`'s overriding `bar` method, the return type is change to `List` which is a super-type to `ArrayList`. This means that the post-condition guraneteed by `Foo` in which an `ArrayList` must be returned is violated. This is because a `List` can implemented as a `Stack`, `LinkedList` and other types that are not `ArrayList`. Thus, the scope of the return type is weaker in the sub-class than super-class and the post-condition is weaken. Hence, violating Liskov Substituion principle, as List is not substituable for ArrayList.

### q5
The design smell present is **divergent change**, this is where 
many changes are made to a single class. Here there is coupling classes being refactored and the Conductor class since by refactoring the conductor class breakdown causing multiple changes to a single class. This is different to shotgun surgery since only one class is constantly breaking. In shotgun surgery refactoring would cause multiple different classes to break down, not just a single class.

### q7
The web server can be designed using the singleton pattern. This creates a single instance of the webserver that would be responsible for booking tickets. 

When a user wants to buy a ticket, the buy() method will be called on that instance and run a sychronized buy method, meaning that only this single thread can access the database, all other remaning threads will be left waiting. This would avoid the issue of tickets being sold that dont exist, which would only occur when mutliple threads are accessing the same ticket in the database. This allows for a multithreaded environment of many different userse attemping to buy tickets, however the synchronized method prevents different threads from accessing the same ticket

### q8
Observer Pattern could be used to model this scenario. The project manager would act as the subject and the stakeholders would act as the observers. This pattern functions by having a list of observers stored within the subject class which allows observers to easily be added and removed from the list. When the project manager updates a project, an update method is called on all the observers in the list sending them a nofitication of the project. 

### q10
Post condition for late penalty in the superclass is range, since "at most", however in the subclass, it is always a fixed late penalty. Thus post condition is strengthened and it is valid